...
<script>
	$(function(){
		...
		//로드뷰 이동
		$("div[name='busStop']").click(function(){
			var bsrpk = $(this).children().eq(0).val();
			document.location.href = "bsLoadView.do?bsrpk="+bsrpk;
		})
		
	});
	
function goBsMap(){
	document.location.href = "/cambus/ybus/jsp/bsMap.jsp?line=circle";
}	
</script>
</head>
<body>
...
<c:choose>
	<c:when test="${ct.count eq '1' }">
 	...
 	<c:when test="${ct.count eq fn:length(timeLine) }">
		...
		<c:set var="whatStopImg" value="/cambus/ybus/images/timeLine/endImg.png"></c:set>
	</c:when>
